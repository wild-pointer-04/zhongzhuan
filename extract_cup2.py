...
class Segmenter:
    """
    交互式分割器。
    - SAM2模式：左键=前景(绿点) 右键=背景(红点) Z=撤销 Enter=确认 ESC=跳过
    - 备用ROI模式：拖拽矩形框选，Enter确认，ESC跳过
    注意：所有OpenCV窗口名只用ASCII，避免Linux下Qt/GTK空指针崩溃。
    """
    ...
    def __init__(self, sam2_ckpt: str = None, sam2_cfg: str = "sam2_hiera_l.yaml"):
        ...
        if self.use_sam2:
            ...
            self.predictor = None
            print("[Seg] SAM2 not available, using ROI box mode")

    def annotate(self, bgr: np.ndarray, frame_idx: int):
        """返回 bool mask (H,W)，用户跳过则返回 None。"""
        if self.use_sam2:
    ...
    def _sam2(self, bgr: np.ndarray, frame_idx: int):
        ...
        cur_mask = None  # 当前SAM2推断的mask
        need_update = False  # 标记是否有新点需要重新推断

        # 窗口名只用ASCII
        WIN = f"SAM2 Frame{frame_idx}"
        ...
        def on_mouse(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                labels.append(1)          # 前景
                need_update = True
                print(f"  [+fg] ({x},{y})  total={len(pts_xy)}")
            elif event == cv2.EVENT_RBUTTONDOWN:
                pts_xy.append((x, y))
                labels.append(0)          # 背景
                need_update = True
                print(f"  [-bg] ({x},{y})  total={len(pts_xy)}")

        cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(WIN, min(bgr.shape[1], 1280), min(bgr.shape[0], 720))
        cv2.setMouseCallback(WIN, on_mouse)

        # 打印操作说明到终端（不用中文写在图上）
        print(f"\n  [Frame {frame_idx}] Annotation window opened.")
        print("  Left-click  = foreground (green dot)")
        print("  Right-click = background (red dot)")
        print("  Z           = undo last point")
        print("  Enter       = confirm & continue")
        print("  ESC         = skip this frame")

        while True:
            # ── 有新点时重新推断mask ──
            if need_update and len(pts_xy) > 0:
                need_update = False
                try:
                    ...
                    cur_mask = masks[best].astype(bool)
            ...
            if cur_mask is not None:
                green_layer = np.zeros_like(disp)
                green_layer[cur_mask] = (0, 230, 0)
                disp = cv2.addWeighted(disp, 0.55, green_layer, 0.45, 0)
                ...
                cv2.drawContours(disp, contours, -1, (0, 255, 80), 2)

            # 画点（前景=绿实心，背景=红实心，外加白色描边）
            for (px, py), lbl in zip(pts_xy, labels):
                color = (0, 255, 0) if lbl == 1 else (0, 0, 255)
                cv2.circle(disp, (px, py), 8, color, -1)
                cv2.circle(disp, (px, py), 8, (255, 255, 255), 1)
                # 中心小点区分
                cv2.circle(disp, (px, py), 2, (255, 255, 255), -1)

            # 状态栏（只用ASCII）
            n_fg = labels.count(1)
            n_bg = labels.count(0)
            status = (f"Frame{frame_idx}  fg={n_fg} bg={n_bg}  "
                      f"mask={'yes' if cur_mask is not None else 'none'}  "
                      f"[L=fg R=bg Z=undo Enter=ok ESC=skip]")
            cv2.putText(disp, status, (8, 24),
            ...
            if key == 13 or key == 32:   # Enter or Space → 确认
                if cur_mask is not None:
                    break
                else:
                    print("  No mask yet, please click on the object first.")
                ...
                if pts_xy:
                    removed_pt  = pts_xy.pop()
                    removed_lbl = labels.pop()
                    need_update = True
                    print(f"  [undo] removed ({'fg' if removed_lbl else 'bg'}) "
                          f"({removed_pt[0]},{removed_pt[1]})  "
                          f"remaining={len(pts_xy)}")
                    if len(pts_xy) == 0:
                        cur_mask = None
    ...
    def _roi(self, bgr: np.ndarray, frame_idx: int):
        # 窗口名只用ASCII
        WIN = f"ROI Frame{frame_idx}"
        print(f"\n  [Frame {frame_idx}] Drag to select ROI, Enter/Space=confirm, ESC=skip")
        cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(WIN, min(bgr.shape[1], 1280), min(bgr.shape[0], 720))

        # selectROI 内部已有鼠标回调，窗口必须先存在
        ...
        print(f"  ROI: x={x} y={y} w={w} h={h}  pixels={mask.sum()}")
        return mask
